...
namespace UnitsNet
{
    ...
    public static class UnitMath
    {
            ...
            where TQuantity : IQuantity
        {
            ...
            {
                return (TQuantity)Quantity.From(new QuantityValue(), unitType);
            }
            return (TQuantity)Quantity.From(source.Sum<TQuantity>(), unitType);
        ...
        /// </summary>
        public static QuantityValue Average<TQuantity>(this IEnumerable<TQuantity> source)
            where TQuantity : IQuantity
        {
            Dimensions dims = source.First().Dimensions;
            double realSum = 0;
            double imaginarySum = 0;
            int count = 0;
            foreach (TQuantity quantity in source)
            {
                if (quantity.Dimensions != dims)
                {
                    throw new ArgumentException($"'{nameof(source)}' cannot have quantities with incompatible dimensions.", nameof(source));
                }
                realSum += quantity.InBaseUnits.Real;
                imaginarySum += quantity.InBaseUnits.Imaginary;
                count++;
            }
            return new QuantityValue(realSum / count, imaginarySum / count);
        }

        /// <summary>
         /// Finds the sum of a set of values.
         /// </summary>
        public static QuantityValue Sum<TQuantity>(this IEnumerable<TQuantity> source)
            where TQuantity : IQuantity
        {
            Dimensions dims = source.First().Dimensions;
            double realSum = 0;
            ...
            return new QuantityValue(realSum, imaginarySum);
        }
            ...
            where TQuantity : IQuantity
        {
            if (source is null || !source.Any())
            {
                throw new InvalidOperationException($"'{nameof(source)}' cannot be empty or null");
            }
            return (TQuantity)Quantity.From(source.Average<TQuantity>(), unitType);
